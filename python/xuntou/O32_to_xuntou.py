...
# cf = ConfigParser.ConfigParser() # python 2.7
...
读取dbf文件
'''
def readBdfFile(filepath, encode=None):    
    # 返回的结果集合
    stocks = []
    # 数据表文件名
...
# 写迅投默认文件单格式文件
def writeFile(filepath, stocks, encode=None):
...
def main():
    while True:
        ...
        for file in file_list:
            ...
            # 读取内容为空，返回
            stocks = readBdfFile(filepath, encode='gbk')
            if (len(stocks) == 0):
                continue
